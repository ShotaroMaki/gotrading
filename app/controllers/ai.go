...
func (ai *AI) Sell(candle models.Candle) (childOrderAcceptanceID string, isOrderCompleted bool) {
	...
	size := ai.AdustSize(availableCoin)
	order := &bitflyer.Order{
		ProductCode: ai.ProductCode,
		ChildOrderType: "MARKET",
		Side: "SELL",
...
func (ai *AI) AdustSize (size float64) float64 {
	fee := size * ApiFeeParcent
	size = size - fee
	//0.1234
	return math.Floor(size*10000)/10000
}

func(ai *AI) WaitUntilOrderComplete(childOrderAcceptanceID string, excuteTime time.Time) bool {
	...
	return func() bool {
		for {
			...
			case <- expire:
				return false
			case <- interval:
				listOrders, err := ai.API.ListOrder(parames)
				if err != nil {
					return false
				}
				if len(listOrders) == 0 {
				...
				if order.ChildOrderState == "COMPLETED" {
					if order.Side == "BUY" {
						couldBuy := ai.SignalEvents.Buy(ai.ProductCode, excuteTime, order.AveragePrice, order.Size, true)
						if !couldBuy {
				...
				if order.Side == "SELL" {
					couldSell := ai.SignalEvents.Sell(ai.ProductCode, excuteTime, order.AveragePrice, order.Size, true)
					if !couldSell {
